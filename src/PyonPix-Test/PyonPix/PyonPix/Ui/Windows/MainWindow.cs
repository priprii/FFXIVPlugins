...
public class MainWindow : BaseWindow
{
	...
	private void DrawHeader()
	{
		...
		SyncSearchWindow syncSearchWindow = Windows.Get<SyncSearchWindow>();
		if (ImGuiEx.IconToggleButton((FontAwesomeIcon)61442, "##openSyncWindow", ((Window)syncSearchWindow).IsOpen, !StateService.LocalPlayerExists || !flag, ((Window)syncSearchWindow).IsOpen ? "Close Sync Search" : "Open Sync Search", null, num4))
		{
			((Window)syncSearchWindow).Toggle();
		}
	...
	private void DrawPixTree()
	{
		...
		foreach (TerritoryData pixTerritory in PixService.GetPixTerritories())
		{
			if (!pixTerritory.MatchesWTWP(currentTerritory))
			{
				DrawTerritoryRow(pixTerritory, isCurrentTerritory: false);
			}
		}
		ImGui.EndChild();
	}
