...
public class PixConfigWindow : BaseWindow
{
	...
	private void DrawSyncTab()
	{
		...
		flag2 |= ImGuiEx.EnumCombo("##editRank", string.Empty, ref value.EditorRank, ComboButtonDisplayType.Items, !flag, "Editor Rank", "The minimum rank required for a user to make changes to synced properties.", 6, 80f * globalScale);
		ImGuiEx.SpacingX(Spacing, sameLinePrior: true, sameLineAfter: true);
		flag2 |= ImGuiEx.Checkbox("Nsfw##nsfw", ref value.Nsfw, !flag, "Nsfw", "Whether this pix may feature mature content.", LineHeight);
		ImGuiEx.Separator(contentRegionAvail.X - Spacing);
		...
		if ((uIState == UIState.Ended || flag2) && flag && NameUtil.ValidatePix(SelectedPix.Info.Name, SelectedPix.Info.Description, value.SecretKey, value.Privacy, SyncService.Client.Premium, out string _))
		{
			ownerFieldBinding.Commit(value);
