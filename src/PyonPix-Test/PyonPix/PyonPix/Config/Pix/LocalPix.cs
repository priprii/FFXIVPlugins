...
public class LocalPix : BasePix
{
	...
	public LocalPix(string id, StateService state)
	{
		...
		base.Renderer.Position = new Vector3(state.LocalPlayerPosition.X, state.LocalPlayerPosition.Y + 1f, state.LocalPlayerPosition.Z);
		base.Renderer.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, (float)Math.PI) * state.LocalPlayerRotation;
