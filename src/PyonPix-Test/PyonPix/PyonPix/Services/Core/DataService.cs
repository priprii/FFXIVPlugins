...
public class DataService(Configuration config, IServiceContext services, IWindowContext windows) : BaseService(config, services, windows)
{
		...
		try
		{
			...
			foreach (string item in Directory.EnumerateDirectories(DataRootPath))
			{
				...
				if (!string.Equals(text, "PIX", StringComparison.OrdinalIgnoreCase))
				{
					...
					PixVariant variant = PixService.GetVariant(pix);
					UDF uDF = new UDF
					{
						FolderName = text,
						FolderPath = item,
						...
						PersistentCache = (variant?.PersistentCache ?? false),
						SizeBytes = -1L,
						LastWriteUtc = null,
						IsRemoving = false,
						PixExists = (pix != null)
	...
	public void SetPersistent(string pixId, bool persistent)
	{
		...
		PixVariant variant = PixService.GetVariant(pixId);
		if (variant != null)
		{
			variant.PersistentCache = persistent;
			Config.Save();
		}
		lock (_lock)
		{
