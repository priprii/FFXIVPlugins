...
public unsafe class PixInputService(Configuration config, IServiceContext services, IWindowContext windows) : BaseService(config, services, windows)
{
	...
	public void HandleImGuiPresentationMouseInput()
	{
		...
		if (BrowserService.FocusedTab == null || !PixService.SpawnedPixs.TryGetValue(BrowserService.FocusedTab.PixId, out IPix value))
		{
			return;
		}
	...
	{
		if (!BrowserService.TryGetRenderBounds(pix, out var _, out var size))
		{
			return Vector2.Zero;
		}
		return uv * size;
	...
	public unsafe void HandleRendererMouseInput(Renderer? renderer, Tab? tab)
	{
		...
		if (renderer == null || tab == null || !PixService.SpawnedPixs.TryGetValue(tab.PixId, out IPix value) || !renderer.ScreenTransform.HasValue || !Matrix4x4.Invert(renderer.ScreenTransform.Value, out var result))
		{
			return;
		}
	...
	private bool HandleRendererMouseEvent(Win32Interop.WM msg, ulong wParam, long lParam)
	{
		...
		case Win32Interop.WM.MBUTTONDOWN:
		{
			...
			if (!IsRendererRegionFocused)
			{
				if (browser.ScreenInteractionReqCtrl && !CtrlPressed)
				{
					return false;
				}
				if (browser.ScreenInteractionReqShift && !ShiftPressed)
				{
					return false;
				}
			}
