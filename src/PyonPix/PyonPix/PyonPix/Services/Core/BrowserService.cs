...
public class BrowserService(PyonPix.Config.Configuration config, IServiceContext services, IWindowContext windows) : BaseService(config, services, windows)
{
	...
	public override Task Initialize()
	{
		...
		ExtensionsService? extensionsService = ExtensionsService;
		...
		ExtensionsService? extensionsService2 = ExtensionsService;
		if (extensionsService2 != null)
		{
			extensionsService2.UninstallExtensionRequest += delegate(string extensionId, string extensionName)
			{
				UninstallExtension(extensionId, extensionName);
			};
		}
		ExtensionsService? extensionsService3 = ExtensionsService;
		if (extensionsService3 != null)
		{
			extensionsService3.EnableExtensionRequest += delegate(string extensionId, string extensionName)
			{
				EnableExtension(extensionId, extensionName);
			};
		}
		ExtensionsService? extensionsService4 = ExtensionsService;
		if (extensionsService4 != null)
		{
			extensionsService4.DisableExtensionRequest += delegate(string extensionId, string extensionName)
			{
	...
	{
		if (u.Pix == null || PixService == null || !PixService.IsSpawned(u.Pix))
		{
			return;
		}
		bool flag;
	...
	public void InitializeBrowser()
	{
		...
		Ipc.OnLog += delegate(Log e)
		{
			...
			case LogType.Verbose:
				...
				break;
			case LogType.Debug:
				Services.Log.Debug("[Browser] " + e.Message, Array.Empty<object>());
				break;
			case LogType.Info:
		...
		Ipc.OnHostInitializeState += delegate(HostInitializeState e)
		{
			...
			case StateType.Success:
				...
				ToggleAutoTheatreMode(Config.Global.Browser.AutoTheatreMode);
				{
		...
		Ipc.OnTabInitializeState += delegate(TabInitializeState e)
		{
			...
			{
				PixVariant pixVariant = PixService?.GetVariant(e.PixId);
				Services.Log.Verbose("[Browser:" + e.PixId + "] Destroyed", Array.Empty<object>());
				if (pixVariant == null || !pixVariant.PersistentCache)
				{
		...
		Ipc.OnHistoryChanged += delegate(HistoryChanged e)
		{
			...
			if (Tabs.TryGetValue(e.PixId, out Tab value))
			{
				...
				else
				{
					...
					{
						if (PixService != null && PixService.SpawnedPixs.TryGetValue(e.PixId, out IPix value2))
						{
		...
		{
			if (Tabs.TryGetValue(e.PixId, out Tab _) && PixService != null && PixService.SpawnedPixs.TryGetValue(e.PixId, out IPix value2) && value2 is SyncedPix syncedPix)
			{
				...
				if (webMessage != null)
				{
					...
					case WebMessageType.MediaState:
					{
						...
						if (mediaState2 != null)
						{
							...
							{
								Services.Log.Debug("[BrowserService] WebMessage MediaState: Client Resync", Array.Empty<object>());
								SyncService?.SyncMediaState(e.PixId, mediaState2);
							}
							else
							{
								syncedPix.Media = mediaState2;
								Services.Log.Debug("[BrowserService] WebMessage MediaState: " + e.Json, Array.Empty<object>());
					...
					{
						Services.Log.Debug("[BrowserService] WebMessage MediaReady: " + e.Json, Array.Empty<object>());
					...
					{
						Services.Log.Debug("[BrowserService] WebMessage MediaResync: " + e.Json, Array.Empty<object>());
					...
					{
						Services.Log.Debug("[BrowserService] WebMessage Navigate: " + e.Json, Array.Empty<object>());
	...
	private void CreateNativeTab(Tab t)
	{
		if (t.State != TabState.Creating && t.State != TabState.Ready)
		{
			...
			string[] extensions = ExtensionsService?.GetExtensionsToInstall() ?? Array.Empty<string>();
			Ipc.SendCreateTab(t.PixId, t.GpuAcceleration, x, y, w, h, t.SyncCookies, extensions);
		}
	}
	...
	private Tab EnsureTabForPix(IPix p)
	{
		...
		PixVariant pixVariant = PixService?.GetVariant(p);
		Tab tab = new Tab
		{
			PixId = p.Id,
			GpuAcceleration = p.Browser.GpuAcceleration,
			SyncCookies = (pixVariant?.SyncCookies ?? false),
	...
	public void UpdateSpatialAudio(string pixId, float left, float right)
	{
		if (State == BrowserState.Running)
		{
			Ipc.SendUpdateSpatialAudio(pixId, left, right);
		}
	}
	...
	{
		if (CanNavigate && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix value))
		{
	...
	public void NavBack()
	{
		if (CanGoBack && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix value))
		{
	...
	{
		if ((CanGoBack || CanGoForward) && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix value))
		{
	...
	{
		if (CanGoForward && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix value))
		{
			FocusedTab.CurrentNavigationIndex++;
			value.Browser.Uri = FocusedTab.CurrentNavigationItem.Uri;
			FocusedTab.NavState = NavigationState.Starting;
	...
	{
		if (CanReload && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix _))
		{
			FocusedTab.NavState = NavigationState.Starting;
	...
	public void ToggleAutoTheatreMode(bool state)
	{
		if (State == BrowserState.Running)
		{
			Ipc.SendToggleAutoTheatreMode(state);
		}
	}

	public void ToggleTheatreMode()
	{
		if (CanNavigate && PixService != null && PixService.SpawnedPixs.TryGetValue(FocusedTab.PixId, out IPix _) && State == BrowserState.Running && FocusedTab.State == TabState.Ready)
		{
