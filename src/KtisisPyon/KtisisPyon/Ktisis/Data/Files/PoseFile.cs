...
public class PoseFile : JsonFile
{
	...
	public static readonly Dictionary<string, string> AnamLegacyConversions = new Dictionary<string, string>
	{
		{ "Root", "n_root" },
		{ "Abdomen", "n_hara" },
		{ "Throw", "n_throw" },
	...
	{
		return Bones?.ContainsKey("j_f_face") ?? false;
	}
	...
	public void ConvertLegacyBones()
	{
		...
		bool flag = false;
		foreach (string item in Bones.Keys.ToList())
		{
			if (AnamLegacyConversions.TryGetValue(item, out string value))
			{
				if (!flag)
				{
					flag = true;
				}
				Bones.Remove(item, out Transform value2);
				if (value2 != null)
				{
					Bones[value] = value2;
				}
